<?php
    ...
    if(empty($_POST['id_apbdes'])){
        echo '<small class="text-danger">ID APBDES tidak boleh kosong</small>';
    }else{
                            ...
                            if($JumlahAnggaranValid==$JumlahValidator){
                                ...
                                for($i=1; $i<$JumlahBaris; $i++){
                                    ...
                                    if(!empty($DataBidangKegiatan['id_bidang_kegiatan'])){
                                        ...
                                        $level=$DataBidangKegiatan['level'];
                                        $nama=$DataBidangKegiatan['nama'];
                                        //Cek apakah data tersebut sudah ada pada database apbdes_rincian
                                        $QryRincian = mysqli_query($Conn,"SELECT * FROM apbdes_rincian WHERE id_apbdes='$id_apbdes' AND kode='$kode'")or die(mysqli_error($Conn));
                                        $DataRincian = mysqli_fetch_array($QryRincian);
                                        if(!empty($DataRincian['id_apbdes_rincian'])){
                                            $id_apbdes_rincian=$DataRincian['id_apbdes_rincian'];
                                            //Update RPJMDES Rincian
                                            $Updateapbdes = mysqli_query($Conn,"UPDATE apbdes_rincian SET 
                                                kode='$kode',
                                                ...
                                                level='$level',
                                                anggaran='$anggaran'
                                            WHERE id_apbdes_rincian='$id_apbdes_rincian'") or die(mysqli_error($Conn)); 
                                            if($Updateapbdes){
                                            ...
                                            //Insert
                                            $entry="INSERT INTO apbdes_rincian (
                                                id_apbdes,
                                                id_wilayah,
                                                kode,
                                                kode_bidang,
                                ...
                                if($JumlahDataProses==$JumlahValidator){
                                    $_SESSION['NotifikasiSwal']="Import Dataset APBDES Berhasil";
