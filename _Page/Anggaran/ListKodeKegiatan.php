    ...
    if(empty($_POST['kode_sub_bidang'])){
        ...
        if(empty($_POST['id_wilayah'])){
            ...
            if(empty($DataWilayah['id_wilayah'])){
                ...
                $query = mysqli_query($Conn, "SELECT * FROM bidang_kegiatan WHERE id_wilayah='$id_wilayah_kabupaten' AND kode_sub_bidang='$kode_sub_bidang' AND level='Kegiatan' ORDER BY kode ASC");
