<?php
    ...
    //Validasi File
    if(empty($_FILES['file_akses_desa']['name'])){
        echo '<code class="text-danger">File tidak boleh kosong</code>';
    }else{
        $nama_file=$_FILES['file_akses_desa']['name'];
        $file_mimes = array('application/octet-stream', 'application/vnd.ms-excel', 'application/x-csv', 'text/x-csv', 'text/csv', 'application/csv', 'application/excel', 'application/vnd.msexcel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        if(isset($_FILES['file_akses_desa']['name']) && in_array($_FILES['file_akses_desa']['type'], $file_mimes)) {
            $arr_file = explode('.', $_FILES['file_akses_desa']['name']);
            ...
            $spreadsheet = $reader->load($_FILES['file_akses_desa']['tmp_name']);
            ...
            if(empty($JumlahValidator)){
                ...
                for($i=1; $i<=$JumlahBaris; $i++){
                    ...
                    if(empty($sheetData[$i]['0'])){
                        ...
                        if(empty($sheetData[$i]['1'])){
                            echo '<li class="text-danger">Baris ke '.$i.' : nama desa tidak boleh kosong</li>';
                        }else{
                ...
                if($JumlahKodeValid!==$JumlahValidator){
                    ...
                    echo '<small class="text-success" id="NotifikasiImportAksesDesaBerhasil">Success</small>';
                }
            }
        }
    }
