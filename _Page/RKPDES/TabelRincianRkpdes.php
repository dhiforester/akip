    ...
    if(empty($_POST['GetIdRkpdes'])){
        ...
        if(!preg_match('/^[0-9]+$/', $id_rkpdes)) {
            ...
            if(empty($DataRkpdes['id_rkpdes'])){
                ...
                $NamaDesa=$DataRkpdes['desa'];
                $NamaKecamatan=$DataRkpdes['kecamatan'];
                $NamaKabupaten=$DataRkpdes['kabupaten'];
                $periode=$DataRkpdes['periode'];
                $kepala_desa=$DataRkpdes['kepala_desa'];
                $sekretaris_desa=$DataRkpdes['sekretaris_desa'];
                $jumlah_anggaran=$DataRkpdes['jumlah_anggaran'];
                $jumlah_anggaran = "" . number_format($jumlah_anggaran, 0, ',', '.');
                $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT * FROM rkpdes_rincian WHERE id_rkpdes='$id_rkpdes'"));
?>
                <div class="row mb-3 mt-3">
                    <div class="col-md-12 text-center">
                        <h3>Rencana Kerja Pemerintah Desa (RKPDES)</h3>
                                ...
                                <tbody>
                                    <?php
                                        if(empty($jml_data)){
                                            ...
                                            //Hitung Jumlah Total
                                            $SqlJumlahTotal = "SELECT SUM(anggaran) AS total FROM rkpdes_rincian WHERE id_rkpdes='$id_rkpdes'";
                                            ...
                                            //Mulai Looping
                                            $no = 1;
                                            $query = mysqli_query($Conn, "SELECT * FROM rkpdes_rincian WHERE id_rkpdes='$id_rkpdes' ORDER BY kode ASC");
                                            while ($data = mysqli_fetch_array($query)) {
                                                $id_rkpdes_rincian= $data['id_rkpdes_rincian'];
                                                $kode= $data['kode'];
