<?php
    ...
    $requiredFields = [
        'id_evaluasi_periode' => "ID Periode Tidak Boleh Kosong!",
    ...
    //Periode Lama
    $periode_lama=GetDetailData($Conn, 'evaluasi_periode', 'id_evaluasi_periode', $id_evaluasi_periode, 'periode');

    //Validasi Tidak Boleh Duplikat
    if($periode_lama==$periode){
        $validasi_duplikat=0;
    }else{
    ...
    // Edit Data
    $stmt_update = mysqli_prepare($Conn, "UPDATE evaluasi_periode SET 
        periode=?, 
        date_mulai=?, 
        date_selesai=? 
    WHERE id_evaluasi_periode=?");
    ...
    if ($update_result) {
        ...
        $SimpanLog=addLog($Conn,$SessionIdAkses,$now,'Periode Evaluasi','Edit Periode Evaluasi');
        if($SimpanLog=="Success"){
            echo json_encode(["status" => "Success", "message" => "Edit Periode Evaluasi Berhasil!"]);
        }else{
